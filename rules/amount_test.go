...
var _ = Describe("Amount Tests", func() {
	...
	DescribeTable("AmountsEqual - Conditions",
		...
		Entry("Invoice missing amount", &types.InvoiceItem{}, &types.ReceivablesItem{Amount: decimalPtr(decimal.New(100, 0))}, false,
			fmt.Errorf("Invoice amount of <nil> does not match receivables amount of 100.00")),
		Entry("Receivables missing amount", &types.InvoiceItem{Amount: decimalPtr(decimal.New(100, 0))}, &types.ReceivablesItem{}, false,
			fmt.Errorf("Invoice amount of 100.00 does not match receivables amount of <nil>")),
		Entry("Invoice and receivables amounts do not match", &types.InvoiceItem{Amount: decimalPtr(decimal.New(100, 0))},
			...
			fmt.Errorf("Invoice amount of 100.00 does not match receivables amount of 200.00")),
